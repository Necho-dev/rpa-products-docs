...
| **连接器代码**   | `rpa.conn.zhifubaotg.dls.item.tmall.conversion.export`             |
| **操作类型**     | `文件导出`                                                         |
| **目标网页**     | `https://adops.alipay.com/report/commodity-analysis/list/physical` |
| **适用场景**     | 使用代理商账号登录支付宝数字推广平台后进入实物商品分析页，打开天猫链路转化归因交易明细，导出交易明细 CSV |
| **数据表名**     | `ods_rpa_zhifubaotg_dls_item_tmall_conversion_export_du`           |
...
![支付宝数字推广平台—实物商品分析天猫链路转化归因入口](../../_public/images/zhifubaotg/dls_item_tmall_conversion_export_20260902.png)

![支付宝数字推广平台—天猫链路转化归因交易明细](../../_public/images/zhifubaotg/dls_item_tmall_conversion_export_detail_20260902.png)
...
| `itemName` | 投放商品名称 | `String` | 是 | `CSV.0.投放商品名称` | `****` (已脱敏) |
| `itemId` | 投放商品ID | `Number` | 是 | `CSV.0.投放商品ID` | `619****683` (已脱敏) |
| `attrUnit` | 归因单元 | `String` | 是 | `CSV.0.归因单元` | `****` (已脱敏) |
| `attrUnitId` | 归因单元ID | `Number` | 是 | `CSV.0.归因单元ID` | `272****811` (已脱敏) |
| `attrCreative` | 归因创意 | `String` | 是 | `CSV.0.归因创意` | `****` (已脱敏) |
