...
| ---------------- | --------------------------------------------------------------------------------------------------- |
| **连接器类型**   | `RPA 连接器`                                                                                        |
| **连接器名称**   | `ODS_商品推广工具操作记录明细表(拼多多RPA)`                                                         |
...
| `custom_start_date` | 查询起始日期 | `String` | 条件必填 | 昨日 | 支持 `YYYYMMDD` 或 `YYYY-MM-DD`；不传日期时使用平台页面默认的昨日，传入时须与 `custom_end_date` 成对，且不能晚于结束日期；仅支持近 30 天（含今天共 30 个自然日） |
