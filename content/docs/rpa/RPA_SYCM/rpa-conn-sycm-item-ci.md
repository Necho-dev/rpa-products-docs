...
![生意参谋—市场—竞争—竞品对比](../_public/images/sycm/item_ci_20260615.png)
...
> 竞品对比页在原有「销售分析 / 来源渠道 / 客群分析」基础上，新增「详情分析」「退款分析」两个主模块 Tab（详情分析带 New 标识）。本次为连接器能力扩展：入参不变，仍沿用 `date_type` / `stat_date`（详情、退款与销售/来源共用；客群仍独立使用 `customer_date_type` / `customer_stat_date`）。
...
| 来源渠道 | 有 | 有（不变） |
| 推荐素材 | 有 | 有（本次不采集） |
| 详情分析 | 无 | **新增**（`ciShop=detail`） |
| 客群分析 | 有 | 有（不变） |
| 退款分析 | 无 | **新增**（`ciShop=rfdAnalysis`） |

**页面改版后截图**

![生意参谋—竞品对比—含详情分析/退款分析](../_public/images/sycm/item_ci_20260727.png)

#### 变更内容

- 追加 Part4 详情分析采集：关键指标对比表、按日趋势、详情模块（含 `children` 嵌套）、主图素材（页面内切换 5 个核心指标：曝光人数 / 互动人数 / 加购转化率 / 支付转化率 / 跳失率）
- 追加 Part5 退款分析采集：退款概况、退款原因；退款原因仅「全部」Tab（落地默认，不点「退货退款」）
- 不进入「素材管理 / 素材测试」外链
- 预估耗时由约 120s 调整为约 180s（含详情素材多指标切换）
