---
title: 店铺-服务数据-商品领航员
description: 采集拼多多商家后台商品领航员列表数据，支持按商品 ID、商品名称筛选
...
| **适用场景**     | 采集拼多多商家后台商品领航员列表数据，支持按商品 ID、商品名称筛选                   |
...
![拼多多商家后台—服务数据—商品领航员](../../public/images/pinduoduo/shop_pilot_goods_20260616.png)

### 业务入参

| 字段 | 中文释义 | 数据类型 | 必填 | 默认值 | 说明 |
| ---- | -------- | -------- | ---- | ------ | ---- |
| `goods_id` | 商品 ID | `string` / `List[string]` | 否 | — | 最多 10 个，英文逗号分隔或 JSON 数组，须为数字 |
| `goods_name` | 商品名称 | `string` | 否 | — | 商品名称关键词 |

### 入参样例

```json
{
    "goods_id": "290328842701",
    "goods_name": "黑咖啡"
}
```
...
| `goodsScoreRkStage` | 综合分阶段（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.goodsScoreRkStage` | `4` |
| `goodsScoreRkStpl` | 综合分数值（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.goodsScoreRkStpl` | `0.7644` |
| `goodsScoreRkStplStage` | 综合分阶段（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.goodsScoreRkStplStage` | `4` |
| `avgDescRevScr1m` | 近 30 天描述评分 | `number` | 否 | `goodsNavigatorDetailDTO.avgDescRevScr1m` | `4.86` |
| `rankAvgDescRevScr1m` | 近 30 天用户品质评价分排名（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankAvgDescRevScr1m` | `0.4727` |
| `rankAvgDescRevScr1mStage` | 近 30 天用户品质评价分排名阶段（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankAvgDescRevScr1mStage` | `3` |
| `rankAvgDescRevScr1mStpl` | 近 30 天用户品质评价分排名（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankAvgDescRevScr1mStpl` | `0.7194` |
| `rankAvgDescRevScr1mStplStage` | 近 30 天用户品质评价分排名阶段（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankAvgDescRevScr1mStplStage` | `4` |
| `qltyRfndOrdrCntRto1m` | 近 30 天品质退款率 | `number` | 否 | `goodsNavigatorDetailDTO.qltyRfndOrdrCntRto1m` | `0.0` |
| `rankQltyRfndOrdrCntRto1m` | 近 30 天品质退款率排名（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankQltyRfndOrdrCntRto1m` | `1.0` |
| `rankQltyRfndOrdrCntRto1mStage` | 近 30 天品质退款率排名阶段（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankQltyRfndOrdrCntRto1mStage` | `4` |
| `rankQltyRfndOrdrCntRto1mStpl` | 近 30 天品质退款率排名（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankQltyRfndOrdrCntRto1mStpl` | `1.0` |
| `rankQltyRfndOrdrCntRto1mStplStage` | 近 30 天品质退款率排名阶段（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankQltyRfndOrdrCntRto1mStplStage` | `4` |
| `ptHelpPzRate30d` | 近 30 天品质求助平台率 | `number` | 否 | `goodsNavigatorDetailDTO.ptHelpPzRate30d` | `0.0` |
| `rankPtHelpPzRate30d` | 近 30 天品质求助平台率排名（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankPtHelpPzRate30d` | `0.9937` |
| `rankPtHelpPzRate30dStage` | 近 30 天品质求助平台率排名阶段（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankPtHelpPzRate30dStage` | `4` |
| `rankPtHelpPzRate30dStpl` | 近 30 天品质求助平台率排名（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankPtHelpPzRate30dStpl` | `0.9956` |
| `rankPtHelpPzRate30dStplStage` | 近 30 天品质求助平台率排名阶段（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankPtHelpPzRate30dStplStage` | `4` |
| `midRto30d` | 近 30 天品质文本情感分 | `number` | 否 | `goodsNavigatorDetailDTO.midRto30d` | `5.0` |
| `rankComscoreStage` | 近 30 天品质文本情感分排名阶段（三级类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankComscoreStage` | `4` |
| `rankComscoreStplStage` | 近 30 天品质文本情感分排名阶段（主营类目） | `number` | 否 | `goodsNavigatorDetailDTO.rankComscoreStplStage` | `4` |
| `goodsAvgAntiDescCtgrpRevScrPct1m` | 商品描述同类评分百分位 | `number` | 否 | `goodsNavigatorDetailDTO.goodsAvgAntiDescCtgrpRevScrPct1m` | `76.09` |
| `mallScoreRkStage` | 店铺体验分阶段 | `number` | 否 | `goodsNavigatorDetailDTO.mallScoreRkStage` | `4` |
...
| `statDate` | 数据统计日期 | `string` | 否 | 列表响应 `result.statDate` | `2026-06-14` |
...
```json
{
    "name": "rpa.conn.pinduoduo.shop.pilot.goods",
