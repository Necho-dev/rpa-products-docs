...
基于 [Fumadocs](https://fumadocs.vercel.app/) + Next.js 构建的文档站点（HeroKnowledge），支持本地开发、静态构建与 Docker 部署。当前版本 **0.5.1**。
...
| 入口 | 行为 |
|------|------|
| 正文里的文档链接 | 桌面端 hover 出预览卡，点击在**右侧并排打开**（非 iframe、不改地址栏）；窄屏为浮层。右栏可前进/后退/复制链接/固定/新标签打开/关闭 |
| 左侧目录、顶栏、面包屑 | 整页跳转到对应文档；右栏**固定**后换页不关右栏 |
| 外链 | 仍弹出操作菜单 |

双栏中间有可拖动手柄，拖动时列宽即时跟随指针。右栏正文与左栏对齐：Copy Markdown、MCP、分享、最后更新。

双栏状态只活在当前会话的 RSC 刷新里，**浏览器刷新不会恢复右栏**。打开双栏时 AI 问答会带上 `layout: split \| sheet` 以及左右栏 `path` / `title` / `url`，便于指「这篇 / 左边 / 右边」。
